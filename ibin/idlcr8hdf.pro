;Main Program Version: idlcr8hdf.pro v4.01, 20110401
;  Written by Ian Boyd, NIWA-ERI for the AVDC - i.boyd@niwa.com
;
;Sub-versions (refer to idlcr8hdf-v4.1_Readme.pdf for full history)
;v1.0        June 2005
;v1.1        August 2005
;v1.11       November 2005
;v2.0        October 2006
;v3.0        March 2008
;v3.01/v3.02 April 2008
;v3.03       May 2008
;v3.04       June 2008
;v3.05       October 2008
;v3.06       March 2009
...
;            vfv removed from DATA; ncsa removed from METADATA - Version 4.01
;
;Input: Nil
;
;Output: Nil
...
ELSE IF (uv EQ 'H4') OR (uv EQ 'H5') THEN o3[0]=uv $
...
;            text regarding the format of the structure required for input - Version 4.01
;
...
vertxt=['idlcr8hdf-v4.01_Readme.pdf','v4.01 April 2011']
errtxt[1]='Welcome to IDLcr8HDF.  This program creates GEOMS compliant HDF4 and compatible HDF5 files'
...
errtxt[9]='  OUTPUT DIRECTORY - Directory for any HDF and log files created. Shortcut values of ''M'' or ''D'''
...
errtxt[18]='  OUTPUT DIRECTORY - Directory for any HDF and log files created.'
...
errtxt[24]='  2. idlcr8hdf,METAFILE,DATAFILE(s),TAVFILE,OUTDIR[,RETERR][,/H5][,/AVK][,/Log][,/Popup]  or'
errtxt[25]='    idlcr8hdf,'''','''','''',''''[,/H5][,/AVK][,/Log][,/Popup]  (For null string, DIALOG_BOXES will prompt for input).'
errtxt[26]='  3. idlcr8hdf,GA,DS,TAVFILE,OUTDIR[,RETERR][,/H5][,/AVK][,/Log][,/Popup] or'
errtxt[27]='    idlcr8hdf,GA,DS[,/H5][,/AVK][,/Log][,/Popup]  (Inputs are from session memory, DIALOG_BOX(s) will'
errtxt[28]='    prompt for input if TAVFILE or OUTDIR are not included).'
errtxt[29]='    /H5, /AVK, /Log and /Popup keywords are used in place of the options given below (HDF4 is default).'
errtxt[31]='Contacts -'
...
wtxt=WIDGET_TEXT(base,xsize=90,ysize=17,/Scroll)
base3=WIDGET_BASE(base,/Nonexclusive)
logtext='Append log output to the file ''idlcr8hdf.log'' (will create the file IF it doesn''t exist)'
...
b4=WIDGET_BUTTON(base3,value=logtext,uvalue='idlcr8hdf.log',frame=3)
...
;  20110401: Change end text dependent on whether the program is creating an HDF file or doing QA
;            on an HDF file - Version 4.01
;
...
ENDIF ELSE BEGIN
  ...
  ENDIF ELSE IF rerr EQ 'NA' THEN BEGIN ;write error to Popup window
    IF o3[4] EQ '0' THEN endtxt='HDF file creation ' $
...
ENDELSE
HEAP_GC
IF rerr EQ 'NA' THEN RETALL $
ELSE BEGIN
  IF o3[4] EQ '0' THEN endtxt='create HDF file' $
  ELSE endtxt='complete GEOMS file test'
  rerr='Unable to '+endtxt+' - '+txtx+txt2
ENDELSE
...
;relevant to the creation of the HDF file. This information can be reported to a Pop-up logging
...
;           READ_METADATA; CHECK_METADATA; EXTRACT_DATA; SET_UP_STRUCTURE; CHECK_STRING_DATATYPE;
;           CHECK_MIN_MAX_FILL; FIND_HDF_FILENAME; AVDC_HDF_WRITE; IDLCR8HDF
;
;Subroutines Called: None
...
var_unit_arr=['%;0;0.01;1','A;0;1;A','C;0;1;s A','cd;0;1;cd','d;0;86400;s','deg;0;1.74533E-2;rad',$
              ...
              'N;0;1E3;m kg s-2','Pa;0;1;m-1 kg s-2','photons;0;1;photons','ppbv;0;1E-9;1',$
              'ppmv;0;1E-6;1','pptv;0;1E-12;1','ppv;0;1;1','psu;0;1;psu','rad;0;1;rad','s;0;1;s',$
              'sr;0;1;sr','V;0;1;m2 kg s-3 A-1','W;0;1;m2 kg s-3','kg;0;1;kg',$
...
;table.dat file used by Envisat or the GEOMS TAV file, with the HDF file generated accordingly.
; ----------
;Written by Ian Boyd, NIWA-ERI for the AVDC - i.boyd@niwa.com
;
;History:
...
;    1. Table Attribute Values file version not identified [939, 943]
;    2. Envisat table.dat ASC2HDF program version not found [983]
;    3. First Envisat table.dat field value not found [992]
;    4. Table Attribute Values file version is not in a valid format [969]
;
...
;       Values file read in) [953]
;    2. Old version of the TAV file in use. Update from http://avdc.gsfc.nasa.gov/Tools [977]
...
;    Possible Conditions for STOP_WITH_ERROR call (plus [line number] where called):
;      1. Syntax of Filename entry incorrect [1051]
;      2. Filename entry not found or not usable [1056]
;      3. File size is too large [1061]
...
;      1. Attribute entry is not GEOMS compliant, and removed from the metadata saved to the HDF file [1217]
;      2. Dataset name renamed according to new DATETIME reporting conventions [1238]
;      3. Double underscore replaced with a single underscore in the variable name [1248]
;      4. VAR_DEPEND value made self-referencing for axis variable [1274]
;      5. Axis variable cannot be dependent on another variable [1284]
;      6. VAR_UNITS=MJD2000 not GEOMS compliant, changed to VAR_UNITS=MJD2K [1302]
;      7. DATA_FILE_VERSION must be in the form 'nnn' [1326, 1335]
;      8. Obsolete Dataset name renamed [1488]
;      9. Obsolete DATA_SOURCE value renamed [1532]
;     10. Obsolete FILE_ACCESS values renamed [1547]
;     11. Rename obsolete VAR_UNITS=DIMENSIONLESS/NONE values [1561]
;     12. DATA_TEMPLATE field is not present in the TAV file [1574]
;     13. DATA_TEMPLATE value renamed based on DATA_SOURCE value [1609]
;
...
;          Inputs: in1=holdvd, in2=vn[vc], in3=resvd[1], in4=av_sel
...
CASE 1 OF
  code EQ 0: BEGIN
      ...
      FOR i=0,nrd-1 DO BEGIN
        ...
        IF facnt NE 0 THEN BEGIN
          ...
          infotxt=infotxt+' Removed from the metadata saved to the HDF file'
          INFOTXT_OUTPUT, infotxt
        ENDIF
      ENDFOR
    END ;Case 0
  ...
  code EQ 2: BEGIN ;Check for and Apply GEOMS rule changes for INDEPENDENT and axis variables
      axis_variables=['DATETIME','LATITUDE','LONGITUDE','ALTITUDE','ALTITUDE.GPH','PRESSURE','DEPTH']
      vnv=STRUPCASE(in2) & vdv=STRUPCASE(in3)
      ;Check that axis-variable is Self_Referencing
      avi=WHERE(vnv EQ axis_variables,avcnt) ;i.e. is the VAR_NAME an axis variable
      IF avcnt NE 0 THEN BEGIN ;possible axis variable so do axis variable checks
        IF avi[0] GT 3 THEN avind=3 ELSE avind=avi[0] ;identifying dimension (time/lat/lon/vertical)
        ...
        test3=avind LE 2 ;VAR_NAME value is one of the 1st three dimension variables
        test4=(avi[0] GE 1) AND (avi[0] LE 2) ;VAR_NAME value is LATITUDE or LONGITUDE
        res=WHERE(STRUPCASE(vn) EQ vnv+'.INSTRUMENT',test5) ;Test whether LATITUDE/LONGITUDE.INSTRUMENT is present
        test6=(vdv EQ 'CONSTANT') AND (avi[0] EQ 0) ;i.e. VAR_NAME=DATETIME and VAR_DEPEND=CONSTANT
        ;check that axis variable not already found in that dimension
        IF in4[avind] EQ '' THEN BEGIN ;do axis variable checks
          IF (test1) OR (test6) THEN BEGIN
            IF vdv NE vnv THEN BEGIN
              infotxt='2 '+in1+' should be self-referencing for axis variable VAR_NAME='+vnv+'/.'
              infotxt=infotxt+' VAR_DEPEND value changed in metadata'
              INFOTXT_OUTPUT,infotxt
              in3=vnv ;self-referencing VAR_DEPEND value
            ENDIF
            in4[avind]=vnv ;valid axis variable so change av_sel to the VAR_NAME
          ENDIF ELSE IF (test2) AND (test4) AND (test5 EQ 0) THEN BEGIN
            ;LATITUDE or LONGITUDE are dependent on another variable so change to
            ;LATITUDE/LONGITUDE.INSTRUMENT if they are not already present
            infotxt='2 Axis variable cannot be dependent on another variable/. '
            infotxt=infotxt+'VAR_NAME='+vnv+' changed to '+vnv+'.INSTRUMENT
            INFOTXT_OUTPUT,infotxt
            in2=vnv+'.INSTRUMENT'
            in4[avind]='I'
          ENDIF ELSE IF (test2) AND (test3) THEN BEGIN
            in4[0]='E1' ;i.e. axis variable cannot be dependent on another variable so return error
          ENDIF
        ENDIF ELSE BEGIN ;must be from vertical dimension so ensure value is not independent or self-referencing
          IF test1 THEN in4[0]='E2' ;i.e. another variable in the vertical dimension is already the axis variable
        ENDELSE
  ...
  code EQ 5: BEGIN ;Look for obsolete DATA_VARIABLES
      ...
      obs_list_1=['AIR.TEMPERATURE','TEMPERATURE.AIR','COLUMN.VERTICAL','H2O.RELATIVE.HUMIDITY',$
                  ...
                  'NO2.COLUMN.TROPOSPHERIC','SEA.SURFACE.TEMPERATURE','OPACITY.ATMOSPHERIC.VERTICAL']
      ...
      rep_list_1=['TEMPERATURE','TEMPERATURE','COLUMN','HUMIDITY.RELATIVE',$
                  ...
                  'NO2.COLUMN.PARTIAL','SEA.SURFACE.TEMPERATURE.SKIN','OPACITY.ATMOSPHERIC']
      ;Obsolete DATA_VARIABLE_01 which need special handling
      obs_list_spec_1=['.AMF','.AVK','.CONCENTRATION']
      ...
      rep_list_conc=['.MIXING.RATIO','.NUMBER.DENSITY','.PARTIAL.PRESSURE']
      ...
      FOR i=0,N_ELEMENTS(obs_list_1)-1 DO BEGIN
        ...
        ENDIF
      ENDFOR
      ;Special cases for DATA_VARIABLE_01 values .AMF, .AVK, and .CONCENTRATION
      FOR i=0,N_ELEMENTS(obs_list_spec_1)-1 DO BEGIN
        ...
        IF rcnt NE 0 THEN BEGIN ;obsolete values found
          ...
          IF i LE 1 THEN BEGIN ;move .AMF and .AVK to DESCRIPTOR section (2 or 3)
            ...
            ENDFOR
          ENDIF ELSE BEGIN
            ;.CONCENTRATION so attempt to identify actual type of measurement (mixing ratio etc) by
            ;looking at corresponding VAR_UNIT values
            mri=WHERE((STRPOS(STRLOWCASE(in3[ri]),'pp') NE -1) OR $
                      (STRPOS(STRLOWCASE(in3[ri]),' g ') NE -1),mrcnt) ;test for mixing ratio
            ...
            IF ccnt EQ 1 THEN BEGIN ;type of measurement successfully found
              ...
              vlen=STRLEN(obs_list_spec_1[i])
              FOR j=0,rcnt-1 DO BEGIN
                IF (STRUPCASE(vn_v[ri[j],0]) EQ 'AIR.CONCENTRATION') AND (ci[0] EQ 1) THEN $
      ...
      FOR i=0,N_ELEMENTS(obs_list_2)-1 DO BEGIN ;Check for obsolete mode values
        ...
        IF rcnt NE 0 THEN BEGIN
          ...
          IF (test1) OR (test2) OR (test3) THEN BEGIN ;special case obsolete values found so replace
            IF (test1) OR (test2) THEN BEGIN
              mi=STRSPLIT(rep_list_2[i],'/',/EXTRACT)
              IF test1 THEN vn_v[ri,1]=mi[0] ELSE vn_v[ri,1]=mi[1]
            ENDIF ELSE vn_v[ri,1]=rep_list_2[i] ;direct replacement
          ENDIF ELSE BEGIN ;all other obsolete mode values
            vn_v[ri,1]=rep_list_2[i] ;direct replacement
  ...
  code EQ 6: BEGIN ;Look for obsolete DATA_SOURCE
      ...
      obs_list=['FTIR_','MICROWAVE.RADIOMETER','LIDAR.BACKSCATTER','LIDAR.DIAL','LIDAR.OLEX',$
                'LIDAR.RIEGL','LIDAR.RMR']
      ;Corresponding replacement list
      rep_list=['FTIR.','MWR.','LIDAR.']
      ...
      spc_list=['C2H6.','CCl2F2.','CCl3F.','CH4.','CHF2Cl.','ClO.','ClONO2.','CO.','CO2.','COF2.',$
                'H2CO.','H2O.','HCFC22.','HCl.','HCN.','HCOOH.','HF.','HNO3.','N2O.','NO.','NO2.',$
                'O3.','OCS.','SF6.','AEROSOL','TEMPERATURE']
      ...
      IF oli NE -1 THEN BEGIN ;obsolete DATA_SOURCE value found
        IF oli GE 2 THEN oli=2 ;to correspond with rep_list index
        ...
        ELSE IF oli EQ 2 THEN BEGIN ;Only Lidar can be Aerosol or Temperature
          fi=WHERE(spc_found NE -1,fcnt)
          IF fcnt NE 0 THEN pri=MIN(spc_found[fi]) ;i.e. Aerosol takes precedence over Temperature
        ENDIF
      ...
      ENDIF ELSE BEGIN
        ...
        IF dcnt EQ 1 THEN BEGIN ;DATA_SOURCE found
          ...
          IF in2[di[0]] NE '' THEN BEGIN
            ...
            res1=STRUPCASE(STRSPLIT(res[0],'.',/EXTRACT))
            ...
            ELSE IF (ccnt GT 1) AND (N_ELEMENTS(res1) GT 1) THEN BEGIN
              ...
              IF ccnt EQ 0 THEN BEGIN ;Try alternatives for H2O and O3
                CASE 1 OF
...
;              VAR_NAME values - Version 4.0
;    2011xxxx: Call additional GEOMS_RULE_CHANGES; Perform checks on HDF4 pre-defined attributes
;
...
;    Possible Conditions for STOP_WITH_ERROR call (plus [line number] where called):
;      1. Metadata value contains too many characters [1408]
;      2. No or invalid DATA_VARIABLES and VAR_NAME attributes present in the metadata [1488]
;      3. DATA_VARIABLES or VAR_NAME value appears more than once [1496,1531]
;      4. No valid Metadata in command line parameter file or arrays [1388,1449,1456]
;      5. VAR_NAME does not match corresponding DATA_VARIABLES value [1517]
;      6. Number of DATA_VARIABLES values does not match the number of VAR_NAME occurrences [1525]
;      7. A required Global Attribute Label is not present in the Metadata [1566]
;      8. A required Global Attribute Label is repeated in the Metadata [1569,1606]
;      9. Number of Attribute occurrences does not match the number of datasets [1639]
;     10. DATA_QUALITY is not present but is a mandatory attribute when DATA_TEMPLATE is reported [1579]
;
;    Information Conditions (when the program is able to make changes):
;      1. Invalid Attribute Label ignored [1440]
;      2. Missing Global Attribute added [1467]
;      3. DATA_VARIABLES attribute added to metadata based on VAR_NAME values [1486]
;      4. Values for DATA_VARIABLES appended to the metadata based on VAR_NAME values [1521]
;      5. VAR_NAME value appended to label based on DATA_VARIABLES value [1541]
;      6. New Global Attribute added [1596]
...
IF ndv EQ 0 THEN BEGIN
  ...
  IF ndv NE 0 THEN BEGIN
    ...
    IF dvfound EQ -1L THEN BEGIN ;increase array sizes by 1 to accommodate DATA_VARIABLES
      ...
      infotxt='1 Missing DATA_VARIABLES Global Attribute/ added to metadata based on VAR_NAME values'
    ENDIF ELSE BEGIN
...
;Check for obsolete FTIR, LIDAR, and MWR DATA_SOURCE_01 values and modify of possible
...
;Procedure called by Check_Metadata to extract the relevant parts of tab_arr and test against a corresponding
;Metadata value.
; ----------
;Written by Ian Boyd, NIWA-ERI for the AVDC - i.boyd@niwa.com
;
...
;  Subroutines Called: STOP_WITH_ERROR (if error state detected)
;    Possible Conditions for STOP_WITH_ERROR call (plus [line number] where called):
;      1. Incorrect number of (sub-)values for a particular Metadata attribute [1700]
;      2. Metadata value does not match any values in the Table Attribute Values file [1730]
...
ENDIF ELSE BEGIN
  ...
  IF gcnt EQ 0 THEN BEGIN ;Check for Case Sensitivities
    ;Any problem with case sensitivity will be fixed in CHECK_METADATA
...
;              LONG64) and change to INTEGER (now means LONG32) for TAV checks - Version 4.0
;
...
;    Possible Conditions for STOP_WITH_ERROR call (plus [line number] where called):
;      1. An attribute value is expected but is absent or invalid [1861,1885,1922,1953]
;      2. Incorrect number of (sub-)values for a particular Metadata attribute [1853,1988,2036]
;      3. NAME and AFFILIATION values don't match (only applies if reading in the full AVDC
;         TAV file) [1915]
;      4. AFFILIATION (AVDC) or DATA_SOURCE_02 (Envisat) field not found in the TAV file [1939]
;      5. There is no match between a given VAR_UNIT, and the list of valid units and prefixes
;         present in the TAV file [2005]
;      6. VAR_UNITS value is not valid [2005]
;
;    Information Conditions (when the program is able to make changes):
;      1. ORIGINATOR values for ADDRESS or EMAIL replaced with those from the TAV file [1907]
;      2. VAR_SI_CONVERSION values replaced with those calculated by the program [2019]
...
FOR i=0,N_ELEMENTS(meta_arr)-1 DO BEGIN
  res=STRSPLIT(meta_arr[i],'=',/Extract,COUNT=nres) & res0=res[0]
  ...
  IF mcnt NE 0 THEN BEGIN ;check Metadata value(s) against TAV file entries
    ...
    ENDIF ELSE IF res[0] EQ 'ORIGINATOR' THEN BEGIN
      ...
      IF STRLEN(rerr) GT 2 THEN RETURN
      IF ti[0] EQ -3 THEN BEGIN ;no match with ORIGINATOR name value so report DEBUG message
        ...
        IF ncnt EQ 1 THEN BEGIN ;first time this name has been found so include message
          ...
          FOR j=1,3 DO BEGIN
            ...
            ENDIF ELSE BEGIN ;add [EMPTY;..] text
              FOR m=0,xval[j]-1 DO BEGIN
                IF m EQ 0 THEN new_entry=new_entry+';[MISSING;' $
          ...
          infotxt[1]='    HDF file values. Note that values may be missing or invalid, or may not'
          infotxt[1]=infotxt[1]+' conform to GEOMS Standard:'
          infotxt[2]='    ='+new_entry
          INFOTXT_OUTPUT,infotxt
        ENDIF
      ENDIF ELSE IF dentry NE res[1] THEN BEGIN ;Case differences between Metadata entry and TAV entry
    ...
    ENDIF ELSE IF res[0] EQ 'AFFILIATION' THEN BEGIN
      ...
      IF STRLEN(rerr) GT 2 THEN RETURN
      IF dentry NE res[1] THEN BEGIN ;Case differences between Metadata entry and TAV entry
        achk2=STRSPLIT(tab_arr[mi[0],ti[0]+2],';',/Extract)
        achk=STRTRIM(achk2[0],2)
    ...
    ENDIF ELSE IF res[0] EQ 'VAR_UNITS' THEN BEGIN
      ...
      IF rd NE 0 THEN BEGIN
        ...
        k=1
        WHILE STRMID(meta_arr[i+k],0,17) NE 'VAR_SI_CONVERSION' DO k=k+1
        achk=STRSPLIT(meta_arr[i+k],'=',/Extract) ;VAR_SI_CONVERSION values
        IF N_ELEMENTS(achk) GE 2 THEN BEGIN
          ;check calculated VAR_SI_VALUE against existing value in the metadata
          IF achk[1] NE tchk THEN BEGIN
            infotxt=STRARR(2)
            IF STRPOS(o3[2],'idlcr8qa.log') NE -1 THEN infotxt[0]='2 '+meta_arr[i+k]+' should be '+tchk+'/' $
            ELSE infotxt[0]='2 VAR_SI_CONVERSION values in Metadata replaced with'
            infotxt[1]='    those from the Table Attribute Value file: '+achk[1]+' -> '+tchk
            INFOTXT_OUTPUT, infotxt
          ENDIF
        ENDIF
    ...
    ENDIF ELSE BEGIN ;perform checks on the remaining ATTRIBUTE_NAMES
      ...
      FOR j=0,mcnt-1 DO BEGIN
        ...
        IF (mcnt EQ 1) AND (dentry NE achk[j]) THEN BEGIN
          ;Case differences between Metadata entry and TAV entry
...
IF inf EQ 1 THEN BEGIN ;Find the first data value for the requested VAR_NAME
  ...
  IF dtest[0] EQ '-1' THEN BEGIN ;need to determine the total number of data lines only (ndl)
    IF NOT EOF(lu) THEN BEGIN
      ;read through until eof or comment line, null line, or another VAR_NAME is reached
      REPEAT BEGIN
        READF,lu,dum
        dum=STRTRIM(STRUPCASE(dum),2)
        bi=WHERE(dum EQ STRUPCASE(vn))
        IF (STRMID(dum,0,1) NE '!') AND (STRMID(dum,0,1) NE '#') AND (dum NE '') AND (bi[0] EQ -1) THEN incnt=1L $
        ELSE incnt=0L
  ...
  ENDIF ELSE BEGIN ;return dtest and ndl
    ...
    dtest=STRTRIM(STRUPCASE(dtest),2)
    ...
    WHILE (bcnt EQ 0) AND (i LE nvn-1) DO BEGIN
      bi=WHERE(dtest EQ STRUPCASE(vn[i]),bcnt)
    ...
    IF bcnt EQ 0 THEN $
      bi=WHERE((STRMID(dtest,0,1) EQ '!') OR (STRMID(dtest,0,1) EQ '#') OR $
               (dtest EQ '') OR (dtest EQ vntest),bcnt)
    ...
    IF (STRMID(dum,0,1) NE '!') AND (STRMID(dum,0,1) NE '#') AND (dum NE '') AND (bi[0] EQ -1) THEN $
      bcnt=1
...
errtxt[4]='VAR_DEPEND variable name(s) must have VAR_DEPEND value of CONSTANT, INDEPENDENT or be self_referencing: '
errtxt[5]='VAR_DEPEND variable name must have matching VAR_SIZE value (= '
errtxt[6]='Data type not BYTE, SHORT, INTEGER, LONG, REAL, DOUBLE, or STRING: '
...
av_sel=STRARR(4) ;array to indicate axis variables in each dimension (changes to VAR_NAME when found in GEOMS_RULE_CHANGES)
  ...
  IF STRMID(meta_arr[lc],0,8) EQ 'VAR_NAME' THEN BEGIN
    ...
    FOR i=1,nd DO BEGIN
      ...
      IF i EQ 1 THEN BEGIN
        ...
        GEOMS_RULE_CHANGES,2,holdvd,vnval,vdval,av_sel
        ...
        ENDIF
        IF vnval NE vn[vc] THEN BEGIN ;VAR_NAME has had .INSTRUMENT appended to it
          ;Rebuild DATA_VARIABLES list and update VAR_NAME value
          vn[vc]=vnval
          meta_arr[lcz]='VAR_NAME='+vnval
          meta_arr[lcdv]='DATA_VARIABLES='
          FOR j=0,nvn-1 DO BEGIN
            IF j EQ 0 THEN meta_arr[lcdv]=meta_arr[lcdv]+vn[j] $
            ELSE meta_arr[lcdv]=meta_arr[lcdv]+';'+vn[j]
          ENDFOR
        ENDIF
        IF STRMID(av_sel[0],0,1) EQ 'E' THEN BEGIN ;error found while attempting to identify axis variables
          CASE 1 OF
            av_sel[0] EQ 'E1': STOP_WITH_ERROR,o3[3]+proname+vname+': ',errtxt[13]+holdvd,lu
            av_sel[0] EQ 'E2': STOP_WITH_ERROR,o3[3]+proname+vname+': ',av_sel[3]+errtxt[14],lu
            av_sel[0] EQ 'E3': STOP_WITH_ERROR,o3[3]+proname+vname+' ',errtxt[15],lu
          ENDCASE
          RETURN
        ENDIF
        resvd[i]=vdval
      ...
      IF gcnt NE 0 THEN BEGIN ;do remaining checks on VAR_DEPEND values
        ...
        IF (vex[1] NE 'CONSTANT') AND (vex[1] NE 'INDEPENDENT') AND (vex[1] NE resvd[i]) THEN BEGIN
          ...
          IF (vcnt NE 0) AND (xcnt EQ 0) AND (av_sel[3] EQ '') THEN BEGIN ;Vertical axis so should be self-referencing
            ...
            av_sel[3]=resvd[i]
          ENDIF ELSE BEGIN
            ;VAR_DEPEND value of the VAR_DEPEND variable name must be INDEPENDENT or an axis variable
            infotxt=STRARR(2)
            infotxt[0]='3 '+meta_arr[lcy]+' variable name(s) for '+vname+' must have VAR_DEPEND'
            infotxt[1]='    value of CONSTANT, INDEPENDENT, or be self-referencing'
            INFOTXT_OUTPUT,infotxt
          ENDELSE
        ENDIF
      ...
      ENDIF ELSE IF (ndm[vc,8] NE 6) AND (N_ELEMENTS(resv) EQ 2) THEN BEGIN
        ...
        ENDIF ELSE BEGIN
          ...
          IF ndm[vc,8] LE 3 THEN BEGIN ;value is integer type
            IF mv_lng[lc] NE 0LL THEN vfvl[i,vc]=mv_lng[lc] ELSE vfvl[i,vc]=LONG64(resv[1])
...
;    20101120: Account for GEOMS rule changes from v3.0 to v4.0
;
;  Inputs: meta_arr - a string array containing the Global and Variable Attributes
...
IF SIZE(dtest,/TYPE) EQ 7 THEN BEGIN ;convert dtest to either LONG64 or DOUBLE for testing
  valid=0
  IF ndm[vc,8] LE 3 THEN dtestx=LONG64(dtest) ELSE dtestx=DOUBLE(dtest)
ENDIF ELSE dtestx=dtest
...
FOR i=0L,ndl+(n_lab-2L) DO BEGIN
  IF i LE n_lab-2L THEN BEGIN
    ...
    IF N_ELEMENTS(res) EQ 2 THEN $
      IF ndm[vc,8] LE 3 THEN lv=mv_lng[i,vc] ELSE lv=mv_dbl[i,vc]
    tcerr=meta_arr[lvi[i]] ;used for type conversion error
  ENDIF ELSE BEGIN
    res=['',''] & lin=n_lab-1L ;label index
    tcerr=STRTRIM(dtest[i-lin],2) ;used for type conversion error
    lv=dtestx[i-lin] ;data value
  ENDELSE
  IF N_ELEMENTS(res) EQ 2 THEN BEGIN
    IF ndm[vc,8] LE 3 THEN mok=CHECK_MATH(lv) ELSE mok=0
    IF mok EQ 32 THEN mok=0 ;Floating-point underflow so ignore
  ENDIF ELSE mok=-1
...
IF vn[vc] EQ 'DATETIME' THEN BEGIN
  ...
  ENDIF
  IF N_ELEMENTS(dtest) GT 1 THEN BEGIN
    ;Are DATETIME values in chronological order?
    dsort=SORT(dtest)
    IF ARRAY_EQUAL(dtest,dtest[dsort]) EQ 0 THEN BEGIN
...
TypeConversionError:
IF valid EQ 0 THEN STOP_WITH_ERROR,o3[3]+proname+vn[vc]+': ',errtxt2[6]+tcerr,lu,ds
...
FOR i=0,N_ELEMENTS(dt)-1 DO BEGIN
  ...
  IF (N_ELEMENTS(res) NE nei) AND (i LE 2) AND (i GE 6) THEN BEGIN
  ...
  IF (i EQ 3) OR (i EQ 4) THEN BEGIN ;calculate the ISO or MJD2000 value as required
    ...
    IF dcnt EQ 0 THEN BEGIN
      di=WHERE(vn EQ 'DATETIME') & etxt='DATETIME '
    ENDIF
    darr=*ds[di[0]].data ;extract the DATETIME values
    IF N_ELEMENTS(darr) EQ 1 THEN itxt=' '
    IF i EQ 3 THEN dtmjd=darr[0] ELSE dtmjd=darr[N_ELEMENTS(darr)-1]
...
IF hdffilename NE res[1] THEN BEGIN
  IF res[1] NE '-1' THEN BEGIN
    infotxt=STRARR(3)
    infotxt[0]='2 Filename determined from Global Attributes does not match'
    infotxt[0]=infotxt[0]+' FILE_NAME entry under File Attributes'
    infotxt[1]='    '+hdffilename+' -> '+res[1]+'/'
...
PRO avdc_hdf5_write, hdffilename, natts, av, range
  ...
  WHILE j LE natts-1 DO BEGIN
    ...
    IF (res EQ 'VAR_DEPEND') OR (res EQ 'VAR_SIZE') THEN BEGIN
      ...
      IF avcnt GT 1 THEN BEGIN
        ...
        av[i,j]=ahold
      ENDIF
      IF res EQ 'VAR_DEPEND' THEN var_depend[i]=av[i,j]
    ENDIF
    ninc=ninc+1 & j=j+1
  ENDWHILE
ENDFOR
...
ENDIF ELSE AVDC_HDF5_WRITE,hdffilename,natts,av,range

END ;Procedure AVDC_HDF_Write



PRO idlcr8hdf, ga, sds, tav, odir, reterr, H5=o1, AVK=o2, LOG=o4, POPUP=o5, QA=o6, NOHDF=o7
...
;
;Program sub-version 4.0 (20101120)
; ----------
;Written by Ian Boyd, NIWA-ERI for the AVDC - i.boyd@niwa.com
;
;  History:
...
;         selection is not valid [3885,3894,3998,4005]]
;      2. If input is via session memory and the array holding the global attributes
;         is not of type string, or is not one dimensional [3907]
...
;         routines (less than v6.2) [3814]
;
;    Information Conditions (when the program is able to make changes):
;      1. /POPUP keyword cannot be used together with the 'reterr' argument [4043]
;      2. Argument 'reterr' must be a scalar variable of type string [4044]
...
ELSE intype=-1
IF (intype NE 7) AND (intype NE 8) AND (intype NE -1) THEN intype=-2
...
ENDIF ELSE BEGIN ;Set options (HDF4, HDF5, AVK, LOGFILE, POP-UP, QA, NOHDF)
  IF KEYWORD_SET(o1) THEN o3=['H5','0','0','D_','0'] ELSE o3=['H4','0','0','D_','0']
...
IF N_PARAMS() GE 5 THEN BEGIN
  ...
  IF SIZE(reterr,/Type) EQ 7 THEN BEGIN
    ...
    IF o3[3] EQ '' THEN BEGIN
      ...
      o3[3]='D_'
      infotxt1[1]='0 /POPUP keyword cannot be used together with the ''reterr'' argument.'
  ...
  ENDIF ELSE BEGIN
    ...
    infotxt2[1]='0 Argument ''reterr'' must be of type string.' ;a returnable variable of type string.'
    infotxt2[2]='  idlcr8hdf will stop normally if an error is encountered.'
  ENDELSE
ENDIF
...
IF (KEYWORD_SET(o1)) AND (FLOAT(!Version.Release) LT 6.2) THEN BEGIN ;No HDF5 library for IDL6.1 or less
  ...
  IF STRLEN(rerr) GT 2 THEN reterr=rerr & RETURN
ENDIF
  ...
  FOR i=0,s_dim[0]-1 DO BEGIN ;s_dim[0] is the number of datasets
    FOR j=0,s_dim[1]-1 DO BEGIN ;s_dim[1] is the number of attributes
      IF pchk[i,j] EQ 1 THEN BEGIN
        ...
        ELSE BEGIN
          vav=*sds[i,j].va_v
          metafile[pcnt]=*sds[i,j].va_l+'='+STRTRIM(vav,2) ;save all values as a string representation
          vavt=SIZE(vav,/TYPE)
...
IF outdir EQ '' THEN BEGIN
  outdir=DIALOG_PICKFILE(/DIRECTORY,Title='Select Directory for Output HDF file(s)', PATH=path)
  IF outdir EQ '' THEN BEGIN
    STOP_WITH_ERROR,'D_'+proname,'HDF file(s) output directory'+errtxt[0],lu
    IF STRLEN(rerr) GT 2 THEN reterr=rerr & RETURN
  ENDIF
ENDIF
...
FOR i=dux[0],dux[1],dux[2] DO BEGIN
  IF (i EQ dux[0]) OR ((i EQ dux[1]) AND (STRPOS(o3[2],'idlcr8hdf.log') NE -1)) THEN BEGIN
    PRINTF,i,'HDF File Input/Output Log - Program Started on '+SYSTIME(0)
    PRINTF,i,''
  ENDIF
ENDFOR
...
FOR i=dux[0],dux[1],dux[2] DO PRINTF,i,'Input TAV File: '+tablefile
...
IF infotxt1[1] NE '' THEN INFOTXT_OUTPUT,infotxt1
IF infotxt2[1] NE '' THEN INFOTXT_OUTPUT,infotxt2
...
FOR ndf=0,nfile-1 DO BEGIN
  IF o3[3] EQ '' THEN BEGIN
    ...
    WIDGET_CONTROL,wtxt,set_value='',/Append
  ENDIF
  FOR i=dux[0],dux[1],dux[2] DO PRINTF,i,''
  ...
  ENDIF ELSE BEGIN
    ...
    FOR i=dux[0],dux[1],dux[2] DO BEGIN
      IF (i EQ dux[0]) OR ((i EQ dux[1]) AND (STRPOS(o3[2],'idlcr8hdf.log') NE -1)) THEN $
  ...
  IF o3[0] EQ 'H5' THEN hdffilename='.h5' ELSE hdffilename='.hdf'
  ...
  IF (STRPOS(o3[2],'idlcr8qa.log') EQ -1) AND (o3[4] EQ '0') THEN BEGIN
    ...
    FOR i=dux[0],dux[1],dux[2] DO PRINTF,i,hdffilename+' successfully created!'
  ENDIF
  ;Free up memory by destroying the heap variables pointed at by its pointer arguments
  PTR_FREE,ds.data
ENDFOR

IF STRPOS(o3[2],'idlcr8qa.log') NE -1 THEN endtxt='HDF QA function ' $
ELSE IF o3[4] EQ '0' THEN endtxt='HDF file creation ' $
ELSE endtxt='GEOMS file testing '
...
IF (rerr EQ '') AND (STRPOS(o3[2],'idlcr8qa.log') EQ -1) AND (o3[4] EQ '0') THEN BEGIN
  IF nfile EQ 1 THEN reterr=hdffilename+' successfully created' $
  ELSE reterr='HDF files successfully created'
ENDIF
