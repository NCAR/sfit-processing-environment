...
def main(argv):
    ...
    #-------------------------------
    nmcHgtFiles = glob( inputs['dataDir'] + 'height' + '/' + str(inputs['year']) + '/ht*.nmc')
    
    if len(nmcHgtFiles) == 0:
        print 'No .nmc files found in: '+ inputs['dataDir'] + 'height' + '/' + str(inputs['year'])
        sys.exit()
    ...
    #------------------------------------
    nmcTempFiles = glob( inputs['dataDir'] + 'temp' + '/' + str(inputs['year']) + '/te*.nmc')
    nmcTempData  = {}
    ...
    if inputs['NonIntrpFileFlg']:
        ...
        # NMC Temp file
        #--------------
        with open(inputs['TempFile'], 'w') as fopen:
            # Write header
            fopen.write('#   LABEL                       Units          Column    Missing Value\n')
            fopen.write('#---------------------------------------------------------------------\n')
            fopen.write('#   Date                        YYYYMMDD       1         NA           \n')
            fopen.write('#   Temperature at 0mb          C              2         -999         \n')
        ...
        if inputs['IntrpFileFlg']:
            ...
            with open(inputs['IntrpFile'], 'w') as fopen:
                # Write header
