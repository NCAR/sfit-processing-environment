...
def main(argv):
    ...
    if int(Inputs['geoms_tmpl']) < 3: Inputs['vTag'] = False
    else: checker(Inputs, ['vTag'])
    
    myhdf = hdfsave.HDFsave(Inputs['gasName'],Inputs['outDir'], Inputs['sfitVer'], Inputs['locID'], Inputs['fileVer'], Inputs['projectID'],  
                            Inputs['geoms_tmpl'], Inputs['geoms_meta'], verTag=Inputs['vTag'], dType='float32', h5Flg=h5Flg, quality=dQuality)
    ...
    if Inputs['yrlFlg']:
        ...
        for year in years:
            ...
            #except: 
            #    print('Error while creating HDF for year {}'.format(year))
            
            if len(myhdf.dates) >=1:
                #--------------------------------------------
